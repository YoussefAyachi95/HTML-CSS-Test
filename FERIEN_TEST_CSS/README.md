...
- [x] Use the "clearfix hack" on the floated element and add a float to the parent element.
...
- [x] Opacity applies transparency to the background color only. Background with an `rgba()` value specifies the level of transparency of an element, as a whole, including its content.
...
- [x] The first column will have a width of 50px. The second column will be 50px wide and the third column will be 100px wide.
...
- [x] Class selectors begin with a leading period
...
- [x] Class selectors are marked with a leading period.
...
- [x] Both paragraphs will be blue.
...
- [x] The color cannot be changed with CSS.
...
- [x] Paragraphs 2 and 3 will be blue. The h2 and paragraph 2 will have a beige background.
...
- [x] Icon fonts increase accessibility.
...
- [x] Icon fonts are inserted as inline images.
...
- [x] There are no errors in the example.
...
- [x] The left margin value is equal to 5% of the selected element's width (.example) plus 5px
...
- [x] `.logo { position: absolute; left-padding: 100px; top-padding: 150px; }`
...
- [x] In CSS3, the double colon notation (`::`) was introduced to create a consistency between pseudo-elements from pseudo-classes. For newer browsers, use the double colon notation. For IE8 and below, using single colon notation (`:`).
...
- [x] in the third panel under the Layout tab
...
- [x] The background image is placed 10% from the top and 50% from the left of its container
...
- [x] The first column is twice the width and height of the second column, and will fit proportionally within the grid container
...
- [x] `transition: color 1.3s ease-in;`
...
- [x] Rules with more specific selectors have greater specificity.
