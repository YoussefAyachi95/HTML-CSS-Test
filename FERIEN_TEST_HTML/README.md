...
- [x] when linking style sheets, external URLs, and favicons
...
- [x] post is used for sending information to the server. get is used for retrieving form information from the server.
...
- [x] It is enclosed in a `<html>` tag.
...
- [x] `itemid="aggregateRating" itemtype="http://schema.org/AggregateRating"`
...
- [x] It creates an ID for a corresponding input element.
...
- [x] The browser plays each sound file in order automatically. The user has controls to stop playback.
...
- [x] It specifies an image that should display while the video downloads.
...
- [ ] `page.html`
- [x] `http://www.linkedin.com/page.html`
- [ ] `dir/page.html`
...
- [x] Wrap the text in a box that is the right width so everything wraps correctly. Set the box width with CSS.
...
- [x] It will display nothing unless the **poster** attribute is set.
...
- [x] It downloads the script from the server when resources allow.
...
- [x] The browser plays the sound automatically and continuously in the background. The user may stop the sound at any tune.
...
- [x] The `<head>` tag contains meta information, while the `<header>` tag contains navigation, logos, and other page identifying content.
...
- [x] `<input type="range" min="0" max="100" by="5" />`
...
- [x] `<style media="light-mode: false">/* CSS code */</style>`
